SELECT TILL_RECEIPT_NUMBER, customersT.CUSTOMER_ID, CU_AGE_RANGE, CU_GENDER, CUSHOM, FSPTS, FSRDM, NUM_ITEMS_IN_BASKET, Grocery, Deli, Tobacco, Produce, Bakery, Salad, Cigarettes, Bottle_deposit, Banana, Lottery
...
IIF(SUM(IIF(SUB_DEPARTMENT in (97), sales_value, NULL))>0, 1,NULL) AS Salad,
IIF(SUM(IIF(SUB_DEPARTMENT in (78), sales_value, NULL))>0, 1,NULL) AS Cigarettes,
IIF(SUM(IIF(UPC in (8810), sales_value, NULL))>0, 1,NULL) AS Bottle_deposit,
IIF(SUM(IIF(UPC in (14), sales_value, NULL))>0, 1,NULL) AS Banana,
